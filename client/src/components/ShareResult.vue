...
<template>
  <section class="transfer-card" aria-label="Link pronto para compartilhar">
    <div class="result-panel">
      ...
      <h2>Seus arquivos já estão protegidos e prontos para serem compartilhados.</h2>
      <div class="result-stats">
        <span
          ><AppIcon name="file" :size="16" /> {{ fileCount }} arquivo(s) · {{ totalLabel }}</span
        >
        <span
          ><AppIcon name="clock" :size="16" />
          {{ expiration === 'one-download' ? '1 download' : '24 horas' }}</span
        >
      </div>
      ...
      <button @click="emit('copy')" class="primary-button">
        <AppIcon :name="copied ? 'check' : 'copy'" :size="18" />
        {{ copied ? 'Link copiado!' : 'Copiar link' }}
      </button>
      <p role="status" class="copy-status">
        {{ copied ? 'Pronto para colar na sua próxima conversa.' : passwordEnabled ? 'Lembre-se de enviar a senha separadamente.' : 'Qualquer pessoa com o link pode baixar.' }}
      </p>
      <p v-if="error" role="alert" class="error-message">{{ error }}</p>
      <button @click="emit('reset')" class="secondary-button">
        <AppIcon name="plus" :size="17" />
        Compartilhar mais arquivos
      </button>
    </div>
  </section>
</template>
