...
export async function ensureStorage() {
  await Promise.all([mkdir(UPLOADS_DIR, { recursive: true }), mkdir(SHARES_DIR, { recursive: true })]);
}
  ...
  await Promise.all(
    names
      ...
      .map(async (name) => {
        ...
        if (!metadata || (metadata.expiresAt && new Date(metadata.expiresAt).getTime() <= Date.now())) {
